...
import { ArrowDownIcon, MinusIcon, PencilIcon, PlusIcon, TrashIcon } from "@heroicons/react/24/solid";

export default function Status({ status, editing, setEditing, updateEntity, addModifier, removeEntity }: Props) {
    ...
    return (
        ...
        {createPortal(
                <Menu id={MENU_ID}>
                    <Item onClick={() => addModifier(status, "add", false)}>{<PlusIcon style={iconStyle} />}{`Add status`}</Item>
                    <Item onClick={() => addModifier(status, "subtract", false)}>{<MinusIcon style={iconStyle} />}{`Subtract status`}</Item>

                    <Item closeOnClick={false} onClick={() => { updateEntity(status.id, (e) => { status.decreaseTier(status.value > 1 ? 1 : 0); return e; }) }} >{<ArrowDownIcon style={iconStyle} />}Decrease tier</Item>
                    <div style={{ display: "flex", flexDirection: "row", justifyContent: "space-between" }}>
                        {[1, 2, 3, 4, 5, 6].map(n => {
                            return (
                                <Item
                                    style={{
                                        border: status.hasTier(n) ? `4px solid ${constant.STATUS_COLOR}` : "4px solid transparent",
                                        borderRadius: "4px"
                                    }}
                                    onClick={() => {
                    ...
                    <Item onClick={() => setEditing(status.id)}>{<PencilIcon style={iconStyle} />}Edit</Item>
                    <Item onClick={() => removeEntity(status)}>{<TrashIcon style={iconStyle} />}Remove</Item>
                </Menu>,
                document.body
            )}
        </>
