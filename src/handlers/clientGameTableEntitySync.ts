...
export function handleClientGameTableEntitySync(
    ...
    console.log(`SYCNING ${entities.length} ENTITIES: ${JSON.stringify(entities)}`)
    const deserialized = entities.map((data) => {return {...data, entity: deserializeRawEntity(data.entity)}})
    setGameTableEntities(() => deserialized)
